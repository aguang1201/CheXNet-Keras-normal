...
def main():
    ...
    model_14 = cp["TRAIN"].get("model_14")
    ...
    try:
        ...
        model = model_factory.get_model(
            ...
            transform_14=True)
        model.load_weights(model_14)
        for layer in model.layers[:427]:
            layer.trainable = False
        x = model.layers[426].output
        ...
        def scheduler(epoch):
            if epoch > 0.9 * epochs:  # 0.8~1:0.00001
                lr = 0.00001
            elif epoch > 0.6 * epochs:  # 0.5~0.8:0.0001
                lr = 0.0001
            elif epoch > 0.3 * epochs:  # 0.5~0.8:0.0001
                lr = 0.001
            else:  # 0~0.5:0.001
                lr = 0.01
